...
不需要用户参与的一种临时登录方式。登录有效期为所获取到的 token 的有效期。
在 token 失效之前，开发者可以使用其他登录方式，将匿名登录用户转变为正式登录用户（可能需要进行合并用户数据操作）。
...
1. 不需要进行用户数据合并
    ...
    匿名登录后，调用 `wx.BaaS.auth.register`，注册返回的 user_id 与之前匿名登录的 user_id 是一致的
    （也就是直接把匿名用户转变为了正式用户），所以不需要数据合并。

2. 可能需要进行用户数据合并

    匿名登录后，调用 `wx.BaaS.auth.loginWithWechat`，由于该微信账号可能已经与平台上的账号进行了绑定（即账号可能已经存在），
    登录成功后，返回的 user_id 与之前匿名登录的 user_id 不一定一致。可以通过对比前后的 user_id 是否一致来判断是否需要数据合并。

3. 需要进行用户数据合并

    匿名登录后，调用 `wx.BaaS.auth.login`，登录成功后，返回的 user_id 必定与之前匿名登录的 user_id 不一致，所以需要数据合并。
...
user 为 currentUser 对象，该对象的某些方法匿名用户无法调用，详细介绍请参考 [currentUser 小节](./account.md)
