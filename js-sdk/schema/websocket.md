...
> - `onevent` 回调函数将在数据表每次有相应动作变化时调用，参数是数据表变化了的数据项
...
另外，在知晓云控制台中手动删除数据时，如需触发删除数据动作的订阅通知，需要勾上「删除动作触发触发器」设置，入下图

![删除数据触发 WebSocket](/images/websocket/dashboard-delete-data.png)

## 取消订阅

当表订阅功能连接成功后，会一直保持订阅状态。如需断开连接，比如退出发起订阅的页面时，可根据需要主动发起取消订阅。
...
下面，我们以**视频弹幕**微信小程序 Demo 为示例，展示表订阅功能在实际开发中的使用。
...
以**我的书架**为例，在数据管理模块，创建一张名为 `danmu_list` 的数据表，并添加一个名为 `text` 的数据列。
...
完成数据表的创建后，我们现在就可以使用知晓云的表订阅功能，对数据进行订阅操作。
...
此外，我们还提供了「我的书架」微信小程序 Demo 站内信版（`hello-world-websocket`），在[新手入门](/newbies/wechat.md)章节中的「我的书架」小程序 Demo 的基础上使用了表订阅功能实现站内信功能，同样可以到[**演示 demo**](https://github.com/ifanrx/hydrogen-demo.git)的 `hello-world-websocket` 文件夹查看。
