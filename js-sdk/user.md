...
> **info**
> 1. 默认情况下，上述内置字段除 `id` 字段外，默认权限设置均为 **客户端可见，且仅创建者可见**
> 2. 针对该权限设置，SDK 中当查询的用户为非当前用户时，上述内置字段仅返回 `id` 字段；当查询的用户为当前用户时，上述内置字段均会返回
> 3. 若开发者需要让 SDK 中非当前用户查询时也能返回某一内置字段，则应修改 `_userprofile` 表中该字段权限为 `仅创建者可见`；若开发者不希望 SDK 中返回某一内置字段，则应修改 `_userprofile` 表中该字段权限为 `客户端不可见`
> 4. 如果有自定义字段，则自定义字段同样根据该字段本身的权限设置，判断是否在 SDK 中进行返回，判断规则同内置字段。
