...
知晓云提供了丰富的 sdk 和 api 支持，每个都有其适用的使用场景，结合不同的sdk 和 api，可以覆盖绝大部分的日常使用场景。

### JS SDK
使用 jssdk 最基础的一个使用场景为：开发者把知晓云 jssdk 集成到小程序中，方便快捷地完成用户注册，信息提交，支付等业务逻辑。

### Node SDK
nodejs SDK 在云函数中使用，可以完成某些在小程序端不方便完成的操作，比如计算密钥，修改不对外公开的数据表等操作。
...
更多关于云函数，nodejs sdk 的文档，请移步这里。

### OPEN API
通过 OPEN API，你可以在自己的服务器上，来操作知晓云的数据。

#### 例子

假设有这样的需求: 每天 0 点遍历知晓云上的 `product` 数据表, 生成 excel 报表。
我们可以在服务器上添加一个 `cron` 任务，定时调用 OPEN API 来获取数据表的数据以便生成报表。

### 运营后台 API
知晓云控制台现有的数据表模块功能复杂且强大，对开发人员友好，但对于运营人员来说，上手使用的难度较大，
...
这时候我可以结合运营后台 API ，搭建一个页面来展示表数据，同时页面上不提供编辑功能，这样既可以满足运营同事的需求，又可以保证数据安全。
